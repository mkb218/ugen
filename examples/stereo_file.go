...
func main() {
	...
	flag.Parse()
	fmt.Println("start!")
	var s0 ugen.UGen = ugen.NewSin(1000, 0, 0, 1)
	...
	// ml.ParamChannel() <- ugen.ParamValue{1, 0}
	// ml.ParamChannel() <- ugen.ParamValue{2, 0}
	// ml.ParamChannel() <- ugen.ParamValue{3, 0}
	
	var ps = ugen.NewAudioParam()
	ps.SetUGenSource(0, sp, 0)
	ps.SetDest(0, s0, 0)

	var o = ugen.NewSndfileOut("stereo_sines.wav", sndfile.Info{Channels:2, Format:sndfile.SF_FORMAT_WAV|sndfile.SF_FORMAT_PCM_24},1, false)
